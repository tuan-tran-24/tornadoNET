...
def gradcam_cnn_lstm(
    ...
    input_tensor = _load_sequence(frame_paths, transform, device)
    ...
    try:
        with torch.enable_grad():
            ...
            if logits.size(1) > 1:
                ...
                loss = (logits[:, predicted_class] - 0.5 * logits[:, negative_class]).sum()
            else:
                loss = logits[:, predicted_class].sum()
